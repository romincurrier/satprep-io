...
- Billing mutation endpoints now enforce a shared browser-origin guard before authenticated billing context is loaded. Public-host requests fail closed without `Origin`, cross-site `Sec-Fetch-Site` requests are rejected, the Origin hostname must match the request host, and non-localhost browser billing traffic must use HTTPS. Checkout creation, checkout confirmation, and billing-portal creation all use the same guard.
- `validate:billing-security` runs in every production build and fails if the shared origin guard or endpoint adoption regresses. The deployment containing this billing hardening passed Vercel.
- Production builds validate parent-progress, administrator-operations, billing mutation security, private-content workflow, content policy, security, privacy, accessibility, adaptive-practice, launch, and regression contracts.
...
6. Exercise billing preview on a non-public host with Stripe test mode, including same-origin success, cross-origin rejection, checkout ownership verification, duplicate-subscription prevention, and portal ownership checks.
7. Keep public launch, live billing, measurement, advertising, outreach, and external publishing disabled until explicit approval and remaining gates are cleared.
