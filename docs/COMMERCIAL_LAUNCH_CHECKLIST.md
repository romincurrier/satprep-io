...
**AI advisory review progress (2026-08-31):** the 434-item expansion bank is now **308/434 AI-reviewed, with 126 remaining**. The formula-backed advisory ledger shows **308 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **130 difficulty changes**, human-review priority of **0 Critical / 43 High / 145 Medium / 120 Normal**, and a reviewed difficulty mix of **184 Easy / 84 Medium / 40 Hard**. This cycle reviewed all 14 `percentages` items. The six nominal-Medium drafts were mostly direct one-step or reverse-percent templates and the four nominal-Hard drafts were one-step discounts/reductions, so ten unapproved staging items were strengthened before review rather than preserving weak difficulty structure. The Medium drafts now include percent-above-100 comparison, two-state reverse percentage, percentage-point versus percent change, changed-denominator part/whole reasoning, successive percentage changes, and percent-plus-additive reverse reasoning. The Hard drafts now use changing composition with changing denominators, subgroup-preserving removal, chained successive-percentage reasoning with reverse inference, and solving an unknown second percentage change relative to an already changed base. The four appropriate Easy items remained unchanged. The six rebuilt Medium items are conservatively routed at Medium human-review priority and the four rebuilt Hard items at High priority. AI QA is advisory only and does not satisfy the independent commercial-review gate.

**Current staging integrity (2026-08-31):** exact post-write checks show **0/434 `production_approved=TRUE`**, no `#REF!` values in the formula-owned `AI Review!A:J` range, and the formula-backed summary independently reports the same 308/126 progress and difficulty/priority totals. Every edited Percentages draft remains `qa_status='draft_unreviewed'`; each advisory review is pinned to the current canonical SHA-256 hash. No reviewed staging item has been independently approved, imported, or activated.
