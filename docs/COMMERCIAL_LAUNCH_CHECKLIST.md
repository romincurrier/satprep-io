...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **114 difficulty changes**, human-review priority of **0 Critical / 91 High / 186 Medium / 157 Normal**, and a reviewed difficulty mix of **213 Easy / 136 Medium / 85 Hard**. QA found a systematic difficulty-progression and construct-quality defect in `command-evidence-textual`: all six author-Medium items had previously calibrated only to Easy and all four author-Hard items had calibrated only to Medium, with the set over-relying on one-step experimental-result matching and weak setup/control distractors. The four appropriate Easy anchors were retained, while the six Medium drafts were rebuilt around multi-step evidence discrimination, source/claim matching, and controlled comparisons, and the four Hard drafts were rebuilt around confound isolation, classification continuity, trade-network diversification, and composition-sensitive biological evidence. Fresh advisory review now classifies all six rebuilt Medium drafts as Medium and all four rebuilt Hard drafts as Hard, with the Hard items High-priority for independent human review. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` errors across the expansion workbook**, exact current SHA-256 binding between each edited `command-evidence-textual` question and its advisory review, and **0 exact or near-duplicate pairs among the 14-item skill set at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately 0.237). The formula-backed summary independently reports the same 434/0 progress and updated difficulty/priority totals. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
