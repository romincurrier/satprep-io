...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **40 difficulty changes**, human-review priority of **0 Critical / 118 High / 159 Medium / 157 Normal**, and a reviewed difficulty mix of **156 Easy / 162 Medium / 116 Hard**. QA found a systematic difficulty-progression defect in `lines-angles-triangles`: all six authored-Medium items were calibrating as Easy and all four authored-Hard items as Medium because they relied on direct triangle-angle subtraction or one multiplication from an explicit similarity scale factor. The four appropriate Easy supplementary-angle anchors were retained. Ten unapproved drafts were substantively rebuilt around exterior-angle algebra, isosceles triangles combined with linear-pair reasoning, parallel-line angle equations, perimeter-derived similarity scale, similar triangles whose full side must first be assembled from segments, parallel-segment similarity with multiple missing sides, reverse reasoning from a perimeter difference, chained corresponding-angle/algebra/triangle-sum deductions, and recovery of an outer segment before a composite-perimeter calculation. Fresh advisory review now classifies all six rebuilt Medium items as Medium and all four rebuilt Hard items as Hard, with the Hard items retained at High priority for independent human review. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` values in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`**, exact current SHA-256 binding between each of the ten revised Lines, Angles, and Triangles staging rows and its advisory-review row, and independent canonical-hash recomputation from the stored/displayed values for all ten revised items. The 14-item Lines, Angles, and Triangles skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately **0.867**, driven by the intentionally parallel Easy supplementary-angle anchors). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and all formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
