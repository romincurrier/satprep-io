...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **12 difficulty changes**, human-review priority of **0 Critical / 122 High / 155 Medium / 157 Normal**, and a reviewed difficulty mix of **132 Easy / 182 Medium / 120 Hard**. QA found a systematic difficulty-progression defect in `right-triangles-trigonometry`: all four authored-Hard items were calibrating as Medium because they were direct sine/cosine/tangent ratio-identification tasks. The four Hard drafts were substantively rebuilt around equal-angle/similarity plus algebra and the Pythagorean theorem, algebraic recovery of side lengths before a trig ratio, two angle-of-elevation tangent models from different observation points, and a sine-derived side relationship followed by a changed-distance tangent condition. Fresh advisory review now classifies all four rebuilt items as Hard. The same within-skill QA also found an exact conceptual/text duplicate between the diagnostic and practice pools; the unapproved Medium practice duplicate was replaced with a distinct Pythagorean-then-cosine item and freshly reviewed as Medium. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, no spreadsheet error markers beginning with `#` in the edited `Questions` or `AI Review` ranges and **0 `#REF!` findings** in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`, exact current SHA-256 binding between each of the five revised Right Triangles and Trigonometry staging rows and its advisory-review row, and independent canonical-hash recomputation from the stored/displayed values for all five revised items. After repairing the pre-existing duplicate, the 14-item Right Triangles and Trigonometry skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately **0.882**). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
