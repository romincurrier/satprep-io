...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **104 difficulty changes**, human-review priority of **0 Critical / 95 High / 182 Medium / 157 Normal**, and a reviewed difficulty mix of **203 Easy / 142 Medium / 89 Hard**. QA found a systematic difficulty-progression and construct-depth defect in `command-evidence-quantitative`: all six author-Medium and all four author-Hard items had previously calibrated to Easy, with the set over-relying on one-step maximum/minimum table comparisons. The four appropriate Easy anchors were retained. The six Medium drafts were rebuilt around linked comparisons across groups, time points, or conditions, subgroup variation, change-versus-level reasoning, and ordering reversals; the four Hard drafts were rebuilt around proportional-versus-absolute change, multi-year gap patterns, unequal-denominator rate reasoning, and an aggregate-versus-within-group reversal. Fresh advisory review now classifies all six rebuilt Medium drafts as Medium and all four rebuilt Hard drafts as Hard, with the Hard items High-priority for independent human review. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` or other formula errors across the expansion workbook**, exact current SHA-256 binding between all 434 staging rows and their advisory-review rows, and exact canonical-hash verification for all 14 `command-evidence-quantitative` items. The 14-item skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately 0.458). A targeted post-write regression caught a spreadsheet array-spill conflict after direct metadata writes temporarily blocked formula-owned `AI Review` columns; the conflicting overrides were removed, the source array formulas were restored, and the summary returned to **434 staged / 434 reviewed / 0 remaining**. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
