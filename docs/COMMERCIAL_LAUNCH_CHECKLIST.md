...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **50 difficulty changes**, human-review priority of **0 Critical / 114 High / 163 Medium / 157 Normal**, and a reviewed difficulty mix of **162 Easy / 160 Medium / 112 Hard**. QA found a systematic difficulty-progression defect in `area-volume`: all six authored-Medium items were calibrating as Easy and all four authored-Hard items as Medium because most reduced to direct rectangular-prism volume substitution or explicit one-step scale-factor calculations. The four appropriate Easy rectangle-area anchors were retained. Ten unapproved drafts were substantively rebuilt around nested-area subtraction, cylinder radius/volume interpretation, dependent prism dimensions, net-to-box spatial reasoning, unit conversion plus tiling area, cubic scaling of similar solids, opposed percentage changes in cylinder dimensions, surface-area-to-volume scale conversion, hollow-prism material volume, and chained surface-area/linear/volume percent reasoning. Fresh advisory review now classifies all six rebuilt Medium items as Medium and all four rebuilt Hard items as Hard, with the Hard items retained at High priority for independent human review. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` values in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`**, exact current SHA-256 binding between each of the ten revised Area and Volume staging rows and its advisory-review row, and independent canonical-hash recomputation from the stored/displayed values for all ten revised items. The 14-item Area and Volume skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately **0.714**, driven by intentionally parallel Easy rectangle-area anchors). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and all formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
