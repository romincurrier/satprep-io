...
**Current advisor status (2026-08-26):** production remains healthy. A fresh security advisor review after the atomic diagnostic finalization migration reports the same two actionable warnings: leaked-password protection is disabled and authenticated users can execute `public.is_admin()` as a SECURITY DEFINER function. Service-only tables with RLS and no browser policy continue to appear as informational lints and remain fail-closed by design. The new diagnostic finalization function is `SECURITY INVOKER` and introduced no new security-advisor warning.
...
1. Add/strengthen secure-v3 acceptance checks that do not require independently reviewed proprietary content, especially answer idempotency, resume/recovery, and parent/student authorization equivalence.
