...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **16 difficulty changes**, human-review priority of **0 Critical / 118 High / 159 Medium / 157 Normal**, and a reviewed difficulty mix of **132 Easy / 186 Medium / 116 Hard**. QA found a systematic difficulty-progression defect in `nonlinear-functions`: all six authored-Medium items were calibrating as Easy because they relied primarily on direct vertex-form recognition. The four appropriate Easy anchors and four already-supported Hard items were retained. Six unapproved Medium drafts were substantively rebuilt around deriving a quadratic extreme value before applying a transformation, recovering a coefficient from an axis-of-symmetry condition, determining exponential parameters from two function values, recovering a quadratic scale from a graph point, inferring symmetry and a vertex from function values, and simplifying a rational function while preserving a removable domain discontinuity. Fresh advisory review now classifies all six rebuilt items as Medium. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 cells containing spreadsheet error markers beginning with `#` in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`**, exact current SHA-256 binding between each of the six revised Nonlinear Functions staging rows and its advisory-review row, and independent canonical-hash recomputation from the stored/displayed values for all six revised items. The 14-item Nonlinear Functions skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately **0.667**). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and all formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns; one transient advisory-row field alignment error detected during regression was corrected immediately before the final checks. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
