...
**AI advisory review progress (2026-08-31):** the 434-item expansion bank is now **336/434 AI-reviewed, with 98 remaining**. The formula-backed advisory ledger shows **336 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **130 difficulty changes**, human-review priority of **0 Critical / 51 High / 157 Medium / 128 Normal**, and a reviewed difficulty mix of **192 Easy / 96 Medium / 48 Hard**. This cycle reviewed all 14 `transitions` items after rechecking the current College Board Expression of Ideas/Transitions definition and public transition-question stem. All 14 generic explanations were replaced with item-specific instructional reasoning. The six nominal-Medium and four nominal-Hard drafts did not provide a meaningful difficulty progression, so ten unapproved staging items were strengthened before review. The Medium set now requires similarity across nonidentical findings, limitation-to-corrective-action purpose, competing route attributes, complementary verification procedures, nominal-versus-usable capacity, and inference across distinct measurement dimensions. The Hard set now requires synthesis of observational and randomized evidence, representativeness limits in a digital archive subset, interpretation of a confounded comparison against a better-controlled follow-up, and composition-dependent interpretation of similar overall averages. The six rebuilt Medium items are conservatively routed at Medium human-review priority and the four rebuilt Hard items at High priority. AI QA is advisory only and does not satisfy the independent commercial-review gate.

**Current staging integrity (2026-08-31):** exact post-write checks show **0/434 `production_approved=TRUE`** and **0 `#REF!` errors across `AI Review`**, the formula-owned `AI Review` metadata array remains healthy and populated across the reviewed rows, and the formula-backed summary independently reports the same 336/98 progress and difficulty/priority totals. Every edited Transitions draft remains `qa_status='draft_unreviewed'`; each advisory review is pinned to the current canonical SHA-256 hash. No reviewed staging item has been independently approved, imported, or activated.
