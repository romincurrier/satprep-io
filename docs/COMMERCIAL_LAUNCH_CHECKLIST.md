...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **124 difficulty changes**, human-review priority of **0 Critical / 87 High / 190 Medium / 157 Normal**, and a reviewed difficulty mix of **219 Easy / 134 Medium / 81 Hard**. QA found a systematic instructional and difficulty-progression defect in `central-ideas-details`: all 14 items reused the same generic explanation, all six author-Medium items had previously calibrated only to Easy, and all four author-Hard items had calibrated only to Medium. All 14 explanations are now item-specific; the six Medium drafts were rebuilt around qualified main ideas, key-detail interpretation, evidence/source limitations, and competing tradeoffs, while the four Hard drafts were rebuilt around archival reinterpretation, time-dependent ecological outcomes, composition-sensitive productivity claims, and multi-source measurement discrepancies. Fresh advisory review now classifies all six Medium drafts as Medium and all four Hard drafts as Hard, with the Hard items High-priority for independent human review. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. The prior `sample-statistics` exam-eligibility correction remains in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` errors across the expansion workbook**, **14/14 unique `central-ideas-details` explanations**, and current SHA-256 review binding for every edited item. The formula-backed summary independently reports the same 434/0 progress and updated difficulty/priority totals. Every staging item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
