...
**AI advisory review progress (2026-08-31):** the 434-item expansion bank is now **420/434 AI-reviewed, with 14 remaining**. The formula-backed advisory ledger shows **420 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **138 difficulty changes**, human-review priority of **0 Critical / 75 High / 192 Medium / 153 Normal**, and a reviewed difficulty mix of **221 Easy / 130 Medium / 69 Hard**. This cycle completed all 14 `probability` items after rechecking the current College Board Problem-Solving and Data Analysis specification for probability and conditional probability. The raw set was systematically templated around direct bag probabilities and repeated conditional-table ratios, while several nominal-Hard items required only a single elementary probability step. All 14 unapproved drafts were diversified or rebuilt before review. The revised set now covers equally likely outcomes, conditional probability from frequency descriptions, overlapping events and inclusion-exclusion, independent compound events, complement reasoning, probability without replacement, and reverse conditioning from subgroup rates. A wording ambiguity in a reverse-conditioning item was caught and repaired before review by explicitly making the Algebra/Geometry groups exhaustive within a two-course program. All 14 pass advisory accuracy, alignment, answer-key, ambiguity, editorial/accessibility, and difficulty checks. The six Medium items remain Medium priority and the four Hard items High priority for independent human review; AI QA remains advisory only and difficulty labels still require empirical calibration. The final 14 outstanding advisory reviews are the `sample-statistics` set.

**Current staging integrity (2026-08-31):** exact post-write checks show **0/434 `production_approved=TRUE`** and **0 `#REF!` errors across `AI Review`**, the formula-owned `AI Review` metadata array remains healthy and the reviewed rows are hash-bound to the current question versions, and the formula-backed summary independently reports the same 420/14 progress and difficulty/priority totals. Every edited Probability draft remains `qa_status='draft_unreviewed'`; each advisory review is pinned to the current canonical SHA-256 hash. No reviewed staging item has been independently approved, imported, or activated.
