...
**AI advisory review progress (2026-08-31):** the 434-item expansion bank is now **392/434 AI-reviewed, with 42 remaining**. The formula-backed advisory ledger shows **392 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **138 difficulty changes**, human-review priority of **0 Critical / 67 High / 180 Medium / 145 Normal**, and a reviewed difficulty mix of **213 Easy / 118 Medium / 61 Hard**. This cycle completed all 14 `one-variable-data` items after rechecking the current College Board Problem-Solving and Data Analysis specification and detailed one-variable-data testing points. The raw set was overly templated around direct mean/median/range arithmetic, including repeated direct-mean formulations, while the nominal Medium/Hard items offered little coverage of distributions or spread. One appropriate Easy direct-mean anchor was retained and the other 13 unapproved drafts were diversified or rebuilt before review. The set now covers median and range, reverse use of the mean relationship, frequency-table weighted means, grouped-histogram median reasoning, effects of an outlier on mean and median, same-center/different-spread comparisons, interpretation of standard deviation without calculating it, an unknown-frequency weighted-mean equation, and combined-group mean reasoning. All 14 pass advisory accuracy, alignment, answer-key, ambiguity, editorial/accessibility, and difficulty checks. The six rebuilt Medium items remain Medium priority and the four rebuilt Hard items High priority for independent human review; AI QA remains advisory only and difficulty labels still require empirical calibration.

**Current staging integrity (2026-08-31):** exact post-write checks show **0/434 `production_approved=TRUE`** and **0 `#REF!` errors across `AI Review`**, the formula-owned `AI Review` metadata array remains healthy and the reviewed rows are hash-bound to the current question versions, and the formula-backed summary independently reports the same 392/42 progress and difficulty/priority totals. Every edited One-Variable Data draft remains `qa_status='draft_unreviewed'`; each advisory review is pinned to the current canonical SHA-256 hash. No reviewed staging item has been independently approved, imported, or activated.
