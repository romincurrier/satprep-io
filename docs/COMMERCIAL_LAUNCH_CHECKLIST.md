...
**Latest full-bank integrity pass (2026-09-01):** a broader 434-item scan found one pre-existing cross-pool duplicate in Advanced Math `systems-equations-two-variables`: the same nonlinear/linear intersection prompt appeared once as diagnostic SPR and once as practice MCQ. The unapproved practice draft was replaced with a distinct Easy nonlinear/linear intersection item and fresh advisory AI QA returned `pass_ai_qa` / Easy; it remains `draft_unreviewed`. The same full-bank canonical recomputation also found **18 stale stored SHA-256 bindings** whose `Questions.content_hash` and `AI Review.content_hash` matched each other but no longer matched the current canonical question content: 10 Text Structure and Purpose items, 2 Boundaries items, 5 Linear Equations in One Variable items, and 1 Two-Variable Data item. Only the two stored hash columns were repaired for those 18 rows; question content and advisory verdicts were not changed. After repair, independent canonical recomputation reports **0/434 stale question hashes and 0/434 question/review hash mismatches**, the formula-backed ledger remains **434 staged / 434 reviewed / 0 remaining / 434 PASS**, there are **0 `#REF!` or other formula-error findings**, and all 434 expansion items remain unapproved. A fresh broad stimulus/stem duplicate scan reports **0 exact or near-duplicate pair at the 0.96 token-Jaccard threshold**; the maximum observed within-bank similarity is approximately **0.933**, below the importer threshold. The staging runbook now requires full-bank canonical recomputation after content-edit batches and explicitly treats same-prompt diagnostic/practice variants as duplicates even when response format differs. Cross-bank screening against the inaccessible separate 94-item bank remains open.
