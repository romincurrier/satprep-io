...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **60 difficulty changes**, human-review priority of **0 Critical / 110 High / 167 Medium / 157 Normal**, and a reviewed difficulty mix of **168 Easy / 158 Medium / 108 Hard**. QA found a systematic difficulty-progression defect in `form-structure-sense`: five authored-Medium items were calibrating as Easy and three authored-Hard items were calibrating as Medium because several nominally advanced questions reduced to one-step agreement, pronoun, tense, or parallel-form choices. The appropriate Easy anchors and already-supported Medium/Hard items were retained. Eight unapproved drafts were substantively rebuilt around agreement across embedded or interrupting clauses, logical modifier attachment, multi-point tense/aspect sequencing, pronoun case inside a relative construction, tense plus coordinated-clause parallelism, and verb complementation across intervening material. Fresh advisory review now classifies every rebuilt item at its authored Medium or Hard level, with rebuilt Hard items retained at High priority for independent human review. The revisions preserve the Standard English Conventions—Form, Structure, and Sense task while making difficulty arise from sentence-level grammatical relationships rather than prose length alone. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` values in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`**, exact current SHA-256 binding between each of the eight revised Form, Structure, and Sense staging rows and its advisory-review row, and independent canonical-hash recomputation from the stored/displayed values for all eight revised items. The 14-item Form, Structure, and Sense skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately **0.350**). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and all formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
