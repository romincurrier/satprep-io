...
- [x] Student-mission parent/student read RLS is InitPlan-safe in production without changing linked-parent/student-self semantics, the separate administrator policy, or authenticated student INSERT/UPDATE policies.
- [x] Weekly-goal parent/student read RLS is InitPlan-safe in production without changing linked-parent/student-self semantics, the separate administrator policy, or authenticated student INSERT/UPDATE policies.
...
- [x] `docs/CONTENT_STAGING_RUNBOOK.md` records the expansion-workbook formula ownership, safe advisory-review write sequence, hash binding, and mandatory post-write regression checks.
...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **0 authored-versus-AI difficulty changes**, human-review priority of **0 Critical / 126 High / 151 Medium / 157 Normal**, and a reviewed difficulty mix of **124 Easy / 186 Medium / 124 Hard**. QA found the final eight remaining under-difficulty Medium drafts across three skills: four `cross-text-connections` items relied on explicit repeated agreement rather than genuine synthesis, two `linear-functions` items reduced to direct unit-rate calculations, and two Algebra `systems-linear-equations` items used favorable one-step elimination. Those eight unapproved drafts were substantively rebuilt around qualified cross-text synthesis/author response, deriving and applying a linear model, contextual system construction, and strategic equation transformation. Fresh advisory review now classifies all eight as Medium. This closes the advisory authored-versus-AI difficulty discrepancy ledger, but it remains AI QA only; no item is represented as independently approved and difficulty still requires empirical calibration.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` findings** in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`, exact current SHA-256 binding between each of the eight revised staging rows and its advisory-review row, and independent canonical-hash recomputation from the stored/displayed values for all eight revised items. Each affected 14-item skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96**; maximum observed token-Jaccard similarity was approximately **0.301 for Cross-Text Connections, 0.875 for Linear Functions, and 0.857 for Systems of Two Linear Equations**. The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.

**Latest full-bank integrity and answer-position pass (2026-09-01):** post-write verification confirms **0/434 stale canonical question hashes, 0/434 question/review hash mismatches, 434/434 advisory reviews present and PASS, 0 production-approved expansion items, and 0 spreadsheet-error values**. A full stimulus/stem duplicate scan still reports **0 exact or near-duplicate pairs at the 0.96 token-Jaccard threshold**, with maximum observed within-bank similarity approximately **0.933**. The existing native-Sheets blank/null rule remains necessary because secondary XLSX paths can materialize true blanks as placeholder values; native Google Sheets values remain authoritative before any stored-hash repair. This cycle found the 10-MCQ `systems-linear-equations` subset concentrated at **A=0 / B=2 / C=6 / D=2** and safely reordered existing choices on three unapproved items, moving two correct options from C to A and one from C to B. The skill now stands at **A=2 / B=3 / C=3 / D=2**. No stimulus, stem, substantive answer, distractor wording, explanation, construct, exam eligibility, or difficulty changed. Fresh advisory answer-key/ambiguity checks remain PASS, the three new canonical hashes match their advisory-review hashes, and all affected rows remain `qa_status='draft_unreviewed'` / `production_approved=FALSE`. The full 364-MCQ expansion distribution is now **A=89 / B=129 / C=99 / D=47**; the broader B-heavy/D-light pattern remains an open staging-QA item for future contained passes, not a basis for changing substantive answers or bypassing independent review. Cross-bank screening against the inaccessible separate 94-item bank remains open.

**Ratios/rates staging QA follow-up (2026-09-01):** the 10-MCQ `ratios-rates-units` subset had a separate local concentration of **A=0 / B=4 / C=6 / D=0**, and two Easy practice drafts repeated the same machine-parts unit-rate template already present in the diagnostic pool closely enough to reduce context/construct variety. The two repetitive unapproved practice drafts were rebuilt as distinct unit-price and mixture-ratio items under the content-staging controls; two other existing MCQs were then choice-reordered without changing their substance. Fresh advisory AI answer-key, ambiguity, alignment, and difficulty checks remain PASS, and all four touched items remain `draft_unreviewed` / `production_approved=FALSE`. The skill's MCQ subset is now **A=2 / B=4 / C=2 / D=2**, and the full 364-MCQ expansion distribution is **A=91 / B=129 / C=95 / D=49**. Full-bank canonical verification also exposed one genuine stale paired hash on `satp-cd2-20260828-two-variable-data-practice-05`; its existing review accurately described the current exponential paired-data item, so only the two stale hash cells were rebound to the current canonical SHA-256 value. Post-repair checks show **0/434 stale canonical hashes, 0 question/review hash mismatches, 434/434 advisory reviews PASS, 0 production-approved expansion items, 0 spreadsheet-error values, and 0 stimulus/stem pairs at or above the 0.96 duplicate threshold**; maximum within-bank similarity remains approximately **0.933**. AI review remains advisory only, independent human review remains mandatory, and no item was imported, activated, or externally published.

**Linear-equations-two-variables staging QA follow-up (2026-09-01):** the 10-MCQ subset was concentrated at **A=6 / B=3 / C=1 / D=0**. Three unapproved items were repaired solely by reordering existing answer choices, moving two correct options from A to D and one from A to C. No stimulus, stem, substantive correct answer, distractor wording, explanation, construct, exam eligibility, or difficulty changed. The subset now stands at **A=3 / B=3 / C=2 / D=2**, and the full 364-MCQ expansion distribution is **A=88 / B=129 / C=96 / D=51**. Fresh advisory AI answer-key and ambiguity rechecks remain PASS; all three items remain `draft_unreviewed` / `production_approved=FALSE`, and AI review is not human approval. Native first-sheet CSV canonical recomputation reports **0/434 stale hashes**, all 434 question hashes match the advisory-review hashes, the workbook error scan finds 0 spreadsheet-error values, and the full stimulus/stem duplicate screen remains at **0 pairs at or above 0.96 token-Jaccard**, with maximum similarity approximately **0.933**. No item was imported, activated, or externally published; the remaining bank-wide B-heavy pattern stays open for later contained QA.

**Form-structure-sense staging QA follow-up (2026-09-01):** the 14-MCQ skill was concentrated at **A=3 / B=9 / C=0 / D=2**. Four unapproved practice items were repaired solely by reordering existing answer choices: three correct options moved from B to C and one moved from B to D. No stimulus, stem, substantive correct answer, distractor wording, explanation, construct, exam eligibility, or difficulty changed. The skill now stands at **A=3 / B=5 / C=3 / D=3**, and the full 364-MCQ expansion distribution is **A=88 / B=125 / C=99 / D=52**. Each item received a fresh advisory AI answer-key and ambiguity recheck and remains `pass_ai_qa`; AI review remains advisory only and is not human approval. The four changed question/review pairs were rebound to freshly recomputed canonical SHA-256 hashes and individually read back after each write. Native first-sheet CSV recomputation reports **0/434 stale question hashes**; the formula-backed AI Review Summary remains **434 reviewed / 434 PASS / 0 remaining / 0 difficulty changes**, with **124 Easy / 186 Medium / 124 Hard**. All 434 expansion rows remain `draft_unreviewed` / `production_approved=FALSE`, a workbook-wide error-string scan found no spreadsheet error values, and the full stimulus/stem duplicate screen remains at **0 pairs at or above 0.96 token-Jaccard**, with maximum similarity approximately **0.933**. No item was imported, activated, or externally published; the remaining bank-wide B-heavy pattern stays open for later contained QA.

**Text-structure-purpose and importer-equivalence follow-up (2026-09-02):** the 14-MCQ `text-structure-purpose` skill was concentrated at **A=5 / B=8 / C=1 / D=0**. Five unapproved items were safely reordered, with three stale letter references in explanations made letter-neutral without changing their substantive rationale. The skill now stands at **A=4 / B=4 / C=3 / D=3**. The same audit found two `boundaries` distractors whose no-punctuation choice depended on leading/trailing whitespace (` ... `). Because the reviewed-content importer trims cell strings before canonical hashing, those two raw-sheet hashes would have failed exact import validation despite matching their advisory-review hashes. The distractors were made explicit as `no punctuation ... no punctuation`; the correct answers (B and C), construct, difficulty, exam eligibility, and rationale remain unchanged, and fresh advisory AI review remains PASS. Post-repair importer-equivalent canonical recomputation reports **0/434 stale hashes**, **0 question/review hash mismatches**, **434/434 advisory reviews PASS**, **0 production-approved expansion items**, **0 spreadsheet-error values**, and **0 stimulus/stem pairs at or above 0.96 token-Jaccard**; maximum within-bank similarity remains approximately **0.933**. The full 364-MCQ expansion distribution is now **A=87 / B=121 / C=101 / D=55**. All 434 expansion rows remain `draft_unreviewed`; no item was independently approved, imported, activated, or externally published. The broader B-heavy/D-light pattern and cross-bank screening against the inaccessible separate 94-item bank remain open.

**Central-ideas-details answer-position follow-up (2026-09-02):** the 14-MCQ `central-ideas-details` skill was concentrated at **A=1 / B=7 / C=6 / D=0**. Four unapproved items were repaired solely by reordering their existing choices: one B-keyed item moved to A, two B-keyed items moved to D, and one C-keyed item moved to D. The four explanations were made letter-neutral so their rationale cannot become stale after a future safe reorder; no stimulus, stem, substantive correct response, distractor wording, construct, exam eligibility, or difficulty changed. Fresh advisory AI answer-key and ambiguity rechecks remain PASS, and all four items remain `draft_unreviewed` / `production_approved=FALSE`; AI review is not human approval. The skill now stands at **A=2 / B=4 / C=5 / D=3**, and the full 364-MCQ expansion distribution is **A=88 / B=118 / C=100 / D=58**. Post-write native first-sheet CSV recomputation reports **0/434 stale canonical hashes**, all 434 question hashes match the advisory-review hashes, the formula-backed summary remains **434 reviewed / 434 PASS / 0 remaining / 0 difficulty changes** with **124 Easy / 186 Medium / 124 Hard**, and the workbook error scan finds **0 spreadsheet-error values**. Duplicate screening remains at **0 pairs at or above 0.96 token-Jaccard**; maximum prompt-only stimulus/stem similarity remains approximately **0.933** and the importer-equivalent stimulus/stem/choice screen peaks at approximately **0.906**. No content was independently approved, imported, activated, or externally published. The remaining bank-wide B-heavy/D-light pattern and cross-bank screening against the inaccessible separate 94-item bank remain open.

**Lines/angles/triangles and inferences answer-position follow-up (2026-09-02):** two remaining local missing-position defects were repaired with four pure existing-choice reorders. The 10-MCQ `lines-angles-triangles` subset moved from **A=0 / B=5 / C=3 / D=2** to **A=2 / B=4 / C=2 / D=2** by moving one Easy diagnostic key C→A and one Medium practice key B→A. The 14-MCQ `inferences` skill moved from **A=4 / B=5 / C=5 / D=0** to **A=4 / B=4 / C=4 / D=2** by moving one Medium diagnostic key B→D and one Medium practice key C→D. No stimulus, stem, substantive correct response, distractor wording, explanation, construct, exam eligibility, or difficulty changed. Fresh advisory AI answer-key and ambiguity rechecks remain PASS and are not human approval; all four rows remain `draft_unreviewed` / `production_approved=FALSE`. Fresh canonical hashes match their corresponding advisory-review hashes. Full post-write regression reports **434 staged / 364 MCQ / 70 SPR, 434/434 advisory PASS, 0 difficulty changes, 124 Easy / 186 Medium / 124 Hard, 0/434 stale hashes, 0 question/review hash mismatches, 0 spreadsheet-error values, 0 exact duplicates, and 0 near-duplicate pairs at or above 0.96 token-Jaccard**. Maximum prompt-only similarity remains approximately **0.933**, and the importer-equivalent screen peaks at approximately **0.906**. The full MCQ answer distribution is now **A=92 / B=110 / C=98 / D=64**. No content was independently approved, imported, activated, or externally published; global exact balance is not being forced, and cross-bank screening against the separate 94-item bank remains open.

**Words-in-context answer-position follow-up (2026-09-02):** the 14-MCQ `words-in-context` skill was **A=3 / B=6 / C=4 / D=1**. Three unapproved items were repaired solely by reordering existing choices, moving their substantive correct responses from B to D while preserving all four response texts. No stimulus, stem, substantive correct response, distractor wording, explanation, construct, exam eligibility, or difficulty changed. Fresh advisory AI answer-key and ambiguity checks remain PASS and are not human approval; all three rows remain `draft_unreviewed` / `production_approved=FALSE`. The skill is now **A=3 / B=3 / C=4 / D=4**, and the full 364-MCQ expansion distribution is **A=90 / B=105 / C=99 / D=70**. Full post-write regression reports **434 staged / 364 MCQ / 70 SPR, 434/434 advisory PASS, 0 difficulty changes, 124 Easy / 186 Medium / 124 Hard, 0/434 stale canonical hashes under native-Sheets blank/null semantics, 0 question/review hash mismatches, 0 spreadsheet-error values, 0 exact duplicates, and 0 pairs at or above 0.96 token-Jaccard**, with maximum prompt-only similarity approximately **0.933**. No content was independently approved, imported, activated, or externally published. Cross-bank screening against the separate 94-item bank remains open.

**Cross-text-connections answer-position follow-up (2026-09-02):** the 14-MCQ `cross-text-connections` skill was **A=2 / B=6 / C=4 / D=2**. Two unapproved diagnostic items were repaired solely by reordering existing choices: `cross-text-connections-diagnostic-02` moved its substantive correct response B→A, and `cross-text-connections-diagnostic-03` moved its substantive correct response B→D. Their explanations were made letter-neutral; no stimulus, stem, substantive correct response, distractor wording, construct, exam eligibility, or difficulty changed. Fresh advisory AI answer-key and ambiguity checks remain PASS and are not human approval; both rows remain `draft_unreviewed` / `production_approved=FALSE`. The skill is now **A=3 / B=4 / C=4 / D=3**, and the full 364-MCQ expansion distribution is **A=91 / B=103 / C=99 / D=71**. Full post-write regression reports **434 staged / 364 MCQ / 70 SPR, 434/434 advisory PASS, 0 difficulty changes, 124 Easy / 186 Medium / 124 Hard, 0/434 stale canonical hashes after applying documented native-Sheets blank/null semantics, 0 question/review hash mismatches, 0 spreadsheet-error values, 0 exact duplicates, and 0 pairs at or above 0.96 token-Jaccard**. Maximum prompt-only similarity remains approximately **0.933**, while the importer-equivalent stimulus/stem/choice screen peaks at approximately **0.906**. The secondary XLSX export again materialized five known true blank stimulus cells in `linear-equations-one-variable` as placeholder `332`; direct native-sheet re-read confirmed those source cells are blank and their existing hashes are correct, so no repair was made. No content was independently approved, imported, activated, or externally published. Cross-bank screening against the separate 94-item bank remains open.
...
**Current pilot execution constraint (2026-09-02):** the authorized Supabase connector still exposes only unrelated project `mirslobrzxdxvkgqlyht` (`Marketing OS Project`) and does not expose SATprep.io production project `ataaiocpbjavmdpgmzlv`. Repository and production-build inspection confirm the deployed self-browser harness still covers parent login, child creation, student activation/login, onboarding, diagnostic, adaptive path, lesson/practice, Journey, parent progress, exact-pilot admin attribution, commercial-content safety, and database isolation checks. This runner still lacks the SATprep production service-only `run_key`/enrollment capability required to invoke the one-shot downstream browser path, and a protected deployment fetch could not mint an access route for a no-key probe. The rendered browser pilot was therefore not bypassed. This remains an access/runner limitation rather than a product failure; normal signup's previously observed production email-delivery/rate-limit blocker also remains pending authorized SATprep production Auth/SMTP access.
...
- [x] Resolve `consent_parties_read` InitPlan finding without changing its PERMISSIVE/default-PUBLIC/SELECT shape, parent-or-student-party visibility, or the separate administrator ALL/parent INSERT policies.
