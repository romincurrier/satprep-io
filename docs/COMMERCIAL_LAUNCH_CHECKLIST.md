...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **75 difficulty changes**, human-review priority of **0 Critical / 106 High / 171 Medium / 157 Normal**, and a reviewed difficulty mix of **178 Easy / 155 Medium / 101 Hard**. QA found a systematic difficulty-progression and construct-depth defect in `text-structure-purpose`: all six author-Medium items had previously calibrated to Easy and all four author-Hard items to Medium because the target sentence's function was usually explicit in isolation. The four appropriate Easy anchors were retained. Ten drafts were substantively rebuilt around source limitations, sampling bias, alternative explanations, confounding, competing mechanisms, provisional interpretive support, source-dependent qualification, and reconciliation of apparently conflicting evidence. Fresh advisory review now classifies all six Medium drafts as Medium and all four Hard drafts as Hard, with the Hard items High-priority for independent human review. The revisions preserve the official Text Structure and Purpose task behavior while making difficulty arise from relating the target sentence to the text's larger rhetorical progression rather than from length alone. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` values in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`**, exact current SHA-256 binding between each revised Text Structure and Purpose staging row and its advisory-review row, and independent canonical-hash recomputation for all ten revised items. The 14-item Text Structure and Purpose skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately 0.226). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and all formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
