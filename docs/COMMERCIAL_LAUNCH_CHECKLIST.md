...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **95 difficulty changes**, human-review priority of **0 Critical / 99 High / 178 Medium / 157 Normal**, and a reviewed difficulty mix of **194 Easy / 147 Medium / 93 Hard**. QA found a systematic difficulty-progression and construct-depth defect in `inferences`: five of the six author-Medium items and all four author-Hard items had previously calibrated to Easy, with the set over-relying on direct mechanical or one-factor causal inferences. The four appropriate Easy anchors and the one appropriately calibrated Medium anchor were retained. Nine drafts were substantively rebuilt around archaeological exchange, edition-to-edition editorial evidence, multi-source chronology, multi-factor ecological evidence, changing literary reception, regional language persistence, transit-navigation evidence with competing changes, genetics-versus-environment classification, and conservation-layer chronology. Fresh advisory review now classifies all six Medium drafts as Medium and all four Hard drafts as Hard, with the Hard items High-priority for independent human review. The official-style inference completion stem is preserved, and harder items require multi-detail synthesis, competing-explanation reasoning, and restraint against unsupported causal or chronological overclaims. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 spreadsheet error values across the expansion workbook**, exact current SHA-256 binding between all 434 staging rows and their advisory-review rows, and exact canonical-hash verification for all 14 `inferences` items. The 14-item skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately 0.194). A targeted post-write regression caught direct writes into formula-owned `AI Review` metadata columns that temporarily blocked the source `ARRAYFORMULA` spill; the conflicting user-entered values were cleared, the source array formulas resumed normally, and the summary returned to **434 staged / 434 reviewed / 0 remaining**. The new `docs/CONTENT_STAGING_RUNBOOK.md` now explicitly restricts advisory writes to `AI Review!B` and `AI Review!K:Y`, leaving formula-owned `A` and `C:J` untouched. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
