...
**AI advisory review progress (2026-08-31):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **134 difficulty changes**, human-review priority of **0 Critical / 83 High / 194 Medium / 157 Normal**, and a reviewed difficulty mix of **225 Easy / 132 Medium / 77 Hard**. QA found a systematic instructional defect in `statistical-claims`: all 14 items reused the same generic explanation, and all four author-Hard items had previously calibrated only to Medium. All 14 explanations are now item-specific, and the four Hard drafts were rebuilt around multi-stage random sampling plus consent, random sampling plus nonresponse in an observational study, a nonrandom comparison-school design, and random sampling plus nonresponse followed by random assignment. Fresh advisory review classifies those four rebuilt items as Hard and High-priority for independent human review. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. The prior `sample-statistics` exam-eligibility correction remains in force.

**Current staging integrity (2026-08-31):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` errors across `AI Review`**, **14/14 unique `statistical-claims` explanations**, zero exact duplicate pairs, and zero near-duplicate pairs at the importer's **0.96 Jaccard threshold** within the 434-item expansion bank. All 434 advisory rows remain hash-bound to their current question versions, and the formula-backed summary independently reports the same 434/0 progress and updated difficulty/priority totals. Every staging item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published.
