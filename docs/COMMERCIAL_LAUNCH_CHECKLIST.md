...
**AI advisory review progress (2026-08-31):** the 434-item expansion bank is now **434/434 AI-reviewed, with 0 remaining**. The formula-backed advisory ledger shows **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **138 difficulty changes**, human-review priority of **0 Critical / 79 High / 198 Medium / 157 Normal**, and a reviewed difficulty mix of **225 Easy / 136 Medium / 73 Hard**. The final `sample-statistics` set has completed advisory accuracy, alignment, answer-key, ambiguity, editorial/accessibility, and difficulty review. Final scope QA found a material exam-eligibility defect: 11 of the 14 items explicitly testing margin of error were tagged for SAT, PSAT/NMSQT, and PSAT 10 even though the current College Board scope makes inference from sample statistics available on SAT/PSAT while margin-of-error testing is SAT-only. Those 11 unapproved drafts are now SAT-only; the three direct sample-mean/proportion estimation items remain eligible for SAT, PSAT/NMSQT, and PSAT 10. The affected items were rebound to new canonical SHA-256 hashes and their advisory review metadata was refreshed. AI QA remains advisory only and difficulty labels still require empirical calibration.

**Current staging integrity (2026-08-31):** exact post-write checks show **0/434 `production_approved=TRUE`** and **0 `#REF!` errors across `AI Review`**. The formula-owned `author_exam_eligibility` array was rechecked after the metadata correction and is expanding normally; all 434 advisory rows remain hash-bound to the current question versions, and the formula-backed summary independently reports the same 434/0 progress and difficulty/priority totals. Every `sample-statistics` item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, or activated.
