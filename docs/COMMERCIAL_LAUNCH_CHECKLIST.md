...
**Latest full-bank integrity and answer-position pass (2026-09-01):** a broader native-sheet verification confirms **0/434 stale canonical question hashes, 0/434 question/review hash mismatches, 434/434 advisory reviews present and PASS, 0 production-approved expansion items, and 0 spreadsheet-error values**. A full stimulus/stem duplicate scan still reports **0 exact or near-duplicate pairs at the 0.96 token-Jaccard threshold**, with maximum observed within-bank similarity approximately **0.933**. A previous secondary XLSX-import verification had materialized true blank Google Sheets cells as placeholder strings and falsely flagged five `linear-equations-one-variable` hashes; native Google Sheets values confirmed those five original hashes were correct, the temporary diagnostic hash writes were reverted, and the answer-key staging runbook now requires native-sheet verification with true blank cells treated as null before any stored-hash repair. The contained answer-position QA pass also repaired six unapproved `command-evidence-quantitative` items solely by reordering existing choices, improving that 14-item skill from **A=10 / B=2 / C=1 / D=1** to **A=4 / B=2 / C=4 / D=4** without changing substantive answers or distractor wording. Fresh advisory answer-key/ambiguity checks remain PASS. The full 364-MCQ expansion distribution is now **A=87 / B=128 / C=102 / D=47**; the broader B-heavy/D-light pattern remains an open staging-QA item for future contained passes, not a basis for changing substantive answers or bypassing independent review. Cross-bank screening against the inaccessible separate 94-item bank remains open.
