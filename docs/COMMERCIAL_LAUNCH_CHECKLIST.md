...
**Current pilot execution constraint (2026-09-01):** the authorized Supabase connector still exposes only unrelated project `mirslobrzxdxvkgqlyht` (`Marketing OS Project`) and does not expose SATprep.io production project `ataaiocpbjavmdpgmzlv`. Repository inspection confirms the deployed self-browser harness can exercise parent login, child creation, student activation/login, onboarding, diagnostic, adaptive path, lesson/practice, Journey, parent progress, and database isolation checks. The direct-trigger wrapper now supplies its internal Cron User-Agent after validating a unique fresh service-only enrollment, so the remaining runner blocker is narrower: this environment does not have the SATprep production service-only `run_key`/enrollment capability needed to invoke that one-shot path safely. The rendered browser pilot was therefore not bypassed. This remains an access/runner limitation rather than a product failure; normal signup's previously observed production email-delivery/rate-limit blocker also remains pending authorized SATprep production Auth/SMTP access.
