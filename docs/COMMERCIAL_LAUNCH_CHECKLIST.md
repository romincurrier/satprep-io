...
**AI advisory review progress (2026-09-01):** the 434-item expansion bank remains **434/434 AI-reviewed, with 0 remaining**. After the latest construct-quality pass, the formula-backed advisory ledger reports **434 PASS / 0 REVISE / 0 REJECT / 0 NEEDS HUMAN REVIEW**, **68 difficulty changes**, human-review priority of **0 Critical / 110 High / 167 Medium / 157 Normal**, and a reviewed difficulty mix of **173 Easy / 156 Medium / 105 Hard**. QA found a systematic difficulty-progression defect in `boundaries`: seven of the 14 items were calibrating below their authored difficulty because several nominal Medium/Hard questions were routine one-step comma, colon, or semicolon decisions. The appropriate Easy anchors and already-supported Medium items were retained. Seven unapproved drafts were substantively rebuilt around long clausal subjects with false pause points, inverted constructions with delayed main verbs, paired boundaries around nested nonessential material, interrupting contrasts, and two-part independent-clause/conjunctive-adverb punctuation decisions. Fresh advisory review now classifies every rebuilt item at its authored Medium or Hard level, with rebuilt Hard items routed to High-priority independent human review. The revisions preserve the Standard English Conventions—Boundaries task while making difficulty arise from sentence structure and competing boundary decisions rather than longer prose alone. This remains AI QA only; no item is represented as independently approved, and difficulty still requires empirical calibration. Prior construct-quality and exam-eligibility corrections remain in force.

**Current staging integrity (2026-09-01):** exact post-write checks show **0/434 `production_approved=TRUE`**, **0 `#REF!` values in `Questions`, `AI Review`, `Coverage`, `Instructions`, `Review Guide`, or `AI Review Summary`**, exact current SHA-256 binding between each revised Boundaries staging row and its advisory-review row, and independent canonical-hash recomputation for all seven revised items. The 14-item Boundaries skill set has **0 exact or near-duplicate pairs at the importer threshold of 0.96** (maximum observed token Jaccard similarity approximately **0.136**). The formula-backed summary remains **434 staged / 434 reviewed / 0 remaining**, and all formula-owned `AI Review` metadata continued to spill normally because writes were confined to the runbook-authorized columns. Every edited item remains `qa_status='draft_unreviewed'`; no reviewed staging item has been independently approved, imported, activated, or externally published. The existing within-bank duplicate/near-duplicate guard remains in force; cross-bank screening against the separately inaccessible 94-item bank is still required before launch.
