...
7. Before importing, run the private readiness preflight against the external file:

   `npm run content:private-readiness -- /absolute/private/path/content-review.csv`

   The preflight prints **metadata counts only**—never question text—and shows per-exam, per-skill diagnostic/practice depth, difficulty distribution, Math SPR counts, excluded rows, and remaining gaps against the current 6-diagnostic / 8-practice commercial policy. Add `--strict` after the file path to make any review/coverage gap blocking.
8. With an active reconciled Supabase project and the server-only service-role credential available locally, import the completed review file with:
...
12. The importer deactivates any existing item before replacing its prompt/key/reviews and only reactivates after all writes complete. The runtime independently recomputes the content hash and requires current approvals, providing another fail-closed layer.
13. Before claiming launch-ready content, run the live strict readiness check. It independently verifies the exact project reference, answer/explanation integrity, exact-hash approvals, reviewer diversity, response-format eligibility, and required per-skill depth/difficulty distribution:

   `npm run verify:launch-content`

## Security and quality properties

- Proprietary question text is never printed by the import or private-readiness scripts.
- Both private-file tools require an **absolute path** and refuse files inside the public repository root.
