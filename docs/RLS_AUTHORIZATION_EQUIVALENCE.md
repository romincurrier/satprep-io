...
Four additional InitPlan-only tranches were verified and applied to production without changing role targets, policy commands, permissiveness, or authorization predicates.
...
Fresh Supabase performance-advisor passes report no `auth_rls_initplan` findings for `public.test_runs`, `public.test_events`, `public.subscriptions`, `public.journey_events`, or the two changed `public.student_achievements` reader policies. The separate `achievement_student_insert` InitPlan warning remains intentionally deferred because it is a write policy and was outside this read-only tranche. A fresh security-advisor pass after the student-achievement change reports no new executable RLS/security regression; the existing fail-closed service-only `rls_enabled_no_policy` informational findings remain, and leaked-password protection remains the separate manual Auth-setting warning.
