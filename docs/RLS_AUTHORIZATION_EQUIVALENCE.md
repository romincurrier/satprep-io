...
Five additional InitPlan-only tranches were verified and applied to production without changing role targets, policy commands, permissiveness, or authorization predicates.
...
Fresh Supabase performance-advisor passes report no `auth_rls_initplan` findings for `public.test_runs`, `public.test_events`, `public.subscriptions`, `public.journey_events`, the two changed `public.student_achievements` reader policies, or the two changed `public.student_missions` reader policies. The separate student write-policy InitPlan warnings on `student_achievements` and `student_missions` remain intentionally deferred because they are outside these read-only tranches. A fresh security-advisor pass after the student-missions change reports no new executable RLS/security regression; the existing fail-closed service-only `rls_enabled_no_policy` informational findings remain, and leaked-password protection remains the separate manual Auth-setting warning.
