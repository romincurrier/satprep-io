...
Seven additional InitPlan-only tranches were verified and applied to production without changing role targets, policy commands, permissiveness, or authorization predicates.
...
Fresh Supabase performance-advisor passes report no `auth_rls_initplan` findings for `public.test_runs`, `public.test_events`, `public.subscriptions`, `public.journey_events`, the two changed `public.student_achievements` reader policies, the two changed `public.student_missions` reader policies, the two changed `public.weekly_goals` reader policies, or `public.parent_rewards.reward_student_read`. The separate student write-policy InitPlan warnings on `student_achievements`, `student_missions`, and `weekly_goals` remain intentionally deferred, and `parent_rewards.reward_parent_all` remains intentionally deferred because it is an ALL/write boundary outside this read-only tranche. A fresh security-advisor pass after the parent-rewards change reports no new executable RLS/security regression; the existing fail-closed service-only `rls_enabled_no_policy` informational findings remain, and leaked-password protection remains the separate manual Auth-setting warning.
