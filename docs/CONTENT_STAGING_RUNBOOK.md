...
For Reading and Writing Cross-Text Connections, make Medium difficulty come from integrating compatible but nonidentical claims, reconciling complementary strengths and limitations, or applying one author's evidentiary criterion or qualification to another text. Do not classify an item as Medium merely because two passages are present when both explicitly repeat the same recommendation or conclusion. Higher-difficulty author-response items should distinguish plausibility from proof, preserve relevant qualifications, and avoid distractors that can be rejected without using both texts.
...
For Math Right Triangles and Trigonometry, make Medium/Hard difficulty come from selecting and chaining right-triangle relationships rather than directly reading a single sine, cosine, tangent, or Pythagorean value from labeled sides. Useful Medium patterns include recovering a missing side before evaluating a trig ratio, applying special-right-triangle structure, or using complementary-angle relationships. Useful Hard patterns include deriving side relationships from algebraic constraints before applying trigonometry, combining similarity or equal-angle ratios with algebra and the Pythagorean theorem, or reconciling two right-triangle models from different observation points. Do not classify direct trig-ratio identification as Hard merely because the item uses symbolic angle notation, and do not repeat the same geometric prompt across diagnostic and practice pools merely by changing response format. Keep right-angle and angle-reference language unambiguous, preserve exam eligibility, and require a unique answer.

For Math Circles, preserve SAT-only eligibility and make Medium/Hard difficulty come from selecting and chaining circle relationships rather than a direct radius/diameter, radius/area, or single arc-fraction calculation. Useful Hard patterns include completing the square before applying radius-tangent perpendicularity, locating a chord endpoint from center-to-chord geometry before finding a tangent slope, eliminating a central-angle variable across sector-area and arc-length relationships, or determining a circle from coordinate geometry before converting between standard and general equation forms. Do not classify a one-step central-angle fraction or radius lookup as Hard merely because the item is SPR or uses π. Keep tangent/chord/arc language unambiguous, preserve SAT-only exam eligibility, and require a unique answer.
...
For Math Linear Functions, make Medium difficulty come from connecting representations or conditions—for example, deriving a linear rule from two points and then applying it to a new or transformed condition—rather than reading a direct unit rate or performing a single substitution. Preserve Easy anchors for direct evaluation and rate reading, and use Hard items for multi-stage transformed-function or implicit-rate reasoning. Keep contextual units and transformed outputs explicit enough to preserve one numeric answer.

For Math Systems of Two Linear Equations in Two Variables under Algebra, make Medium difficulty come from translating a two-condition context into a system, strategically transforming an equation before elimination, or solving the system and then evaluating a requested expression. Do not classify a pre-aligned one-step elimination with small integer coefficients as Medium merely because two equations are shown. Keep the separate Advanced Math nonlinear-system skill distinct, and preserve a unique requested solution or derived quantity.
