# SATprep.io Public Content Source Notes

Last verified: 2026-08-24

Purpose: keep factual SAT/PSAT claims on public pages traceable to current first-party sources. This file is a content-governance reference, not permission to reproduce College Board question text. Reverify time-sensitive test-structure facts before major seasonal/public launches.

## Primary official sources

### SAT Reading and Writing overview
Official source: College Board, “The Reading and Writing Section”
`https://satsuite.collegeboard.org/sat/whats-on-the-test/reading-writing`

Verified facts used in SATprep.io public copy:
- Reading and Writing uses short passages or passage pairs followed by a single multiple-choice question.
- Passages range from about 25 to 150 words.
- The four domains are Information and Ideas, Craft and Structure, Expression of Ideas, and Standard English Conventions.
- Both modules include questions from all four domains.

### Reading and Writing specifications
Official source: College Board, “Reading and Writing Specifications”
`https://satsuite.collegeboard.org/k12-educators/about/alignment/reading`

Verified facts used in SATprep.io public copy:
- Standard SAT Reading and Writing section: 54 total questions.
- Two modules of 32 minutes each, 64 minutes total.
- Current domain distribution published by College Board is approximately:
  - Information and Ideas: 26%
  - Craft and Structure: 28%
  - Expression of Ideas: 20%
  - Standard English Conventions: 26%
- Information and Ideas skill points include Central Ideas and Details, Command of Evidence, and Inferences.
- Craft and Structure includes Words in Context, Text Structure and Purpose, and Cross-Text Connections.
- Expression of Ideas includes Rhetorical Synthesis and Transitions.
- Standard English Conventions includes Boundaries and Form, Structure, and Sense.

### SAT Suite content domains
Official source: College Board, “Content Domains”
`https://satsuite.collegeboard.org/higher-ed-professionals/sat-validity/content-domains`

Verified facts used in content architecture/public copy:
- Math has four main domains: Algebra, Advanced Math, Problem-Solving and Data Analysis, and Geometry and Trigonometry for the SAT.
- Algebra skill points include linear equations in one variable, linear equations in two variables, linear functions, systems of two linear equations, and linear inequalities.
- Advanced Math includes equivalent expressions, nonlinear equations, systems, and nonlinear functions.
- Problem-Solving and Data Analysis includes ratios/rates/units, percentages, one-variable data, two-variable data, probability, inference/sample statistics, margin of error and evaluation of statistical claims subject to exam eligibility.
- Geometry and Trigonometry includes area/volume, lines/angles/triangles, right triangles/trigonometry, and circles subject to exam eligibility.

### SAT Math type pages
Official sources:
- `https://satsuite.collegeboard.org/sat/whats-on-the-test/math/types`
- `https://satsuite.collegeboard.org/sat/whats-on-the-test/math/types/advanced`
- `https://satsuite.collegeboard.org/sat/whats-on-the-test/math/types/problem-solving`
- `https://satsuite.collegeboard.org/sat/whats-on-the-test/math/types/geometry-trigonometry`

Use these pages to reverify domain-level Math copy before updating public skill-cluster pages.

### College Board Student Question Bank stem patterns
Official source: College Board, “Student Question Bank: Reading and Writing Questions”
`https://satsuite.collegeboard.org/practice/student-question-bank/reading-writing`

Governance rule:
- This source may inform understanding of current task types and official skill labels.
- Do **not** copy, scrape, or lightly paraphrase protected official question content into the SATprep.io proprietary bank.
- SATprep.io questions must remain original and pass originality review.

## Public-copy claim rules
Safe factual claim types when verified against current official sources:
- current section/module timing and question count;
- official domain and skill names;
- high-level descriptions of what a domain measures;
- neutral explanation of how SATprep.io organizes study around those skills.

Claims requiring internal evidence before publication:
- score increases;
- average improvement;
- faster learning;
- superior outcomes versus a competitor;
- admission/scholarship outcomes;
- predictive accuracy of SATprep.io mastery or score estimates.

Never publish unsupported guarantees.

## Trademark/non-affiliation rule
Public pages should clearly state that SATprep.io is independent and is not endorsed or sponsored by College Board. SAT® and College Board® are trademarks of College Board. Final legal/trademark language remains subject to launch review.

## Refresh process
Before a major annual or test-cycle content refresh:
1. Open each official source above.
2. Confirm timing, question counts, domain names, skill labels, and exam-specific eligibility.
3. Update `sat-spec.js` and public pages together if a specification changed.
4. Run the content, SEO, approval, and security build validators.
5. Record the new verification date here.
6. Re-review any proprietary question whose taxonomy/exam eligibility changed.

If an official source conflicts with old SATprep.io copy, treat the old copy as stale until resolved. Do not preserve an outdated claim simply for SEO continuity.
