...
const rows=Object.entries(SKILL_INDEX).map(([skill,meta])=>{
 const diagnostic=count(QUESTION_BANK,skill),practice=count(PRACTICE_BANK,skill),stagedPractice=count(STAGED_PRACTICE_BANK,skill),approvedDiagnostic=count(QUESTION_BANK,skill,q=>q.status==='production_approved'),approvedPractice=count(PRACTICE_BANK,skill,q=>q.status==='production_approved');
...
console.log(`Skills at staged launch-depth target: ${readyDepth}/${total}. Skills with minimum production-approved depth: ${readyApproved}/${total}.`);
console.table(rows);
console.log('\nImportant: staged items are NOT student-facing until QA is cleared. Item counts are not a quality substitute. “production_approved” must require independent answer-key, explanation, taxonomy/alignment, originality, accessibility, and editorial review.');
