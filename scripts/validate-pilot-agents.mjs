...
const core=read('server/pilot-agent-core.js');
...
for(const key of ['public_indexing','public_billing','live_payments','first_party_measurement','outbound_marketing']){
 need(gates[key]===false,`Launch gate ${key} must remain false while pilot agents are enabled.`);
}
...
console.log('Pilot-agent safety validation passed: synthetic identity boundaries, admin-only mutations, content isolation, and commercial launch gates remain enforced.');
