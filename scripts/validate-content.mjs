...
const PRACTICE_BANK=[...BASE_PRACTICE_BANK,...EXTRA_PRACTICE_BANK.slice(0,16)];
...
for(const q of PRACTICE_BANK){if(diagnosticIds.has(q.id))errors.push(`Practice item duplicates diagnostic id: ${q.id}`);if(diagnosticFingerprints.has(fingerprint(q)))errors.push(`Practice item duplicates a complete diagnostic item: ${q.id}`);if(q.origin!=='satprep_original_practice')errors.push(`${q.id}: practice origin must be satprep_original_practice`)}
...
console.log('Staged practice QA probe passed for first 16 extra items.');
