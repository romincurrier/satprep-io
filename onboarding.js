...
async function authedPost(url,body){
  ...
  const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify(body)}),data=await r.json().catch(()=>({}));
...
function childForm(title="Tell us about your child",additional=false){
  return `<div class="eyebrow">${additional?"ADD ANOTHER STUDENT":"STUDENT SETUP"}</div><h2>${esc(title)}</h2><p class="small">${additional?"Enter one additional student below.":"Let's start with one student. If you need another, you can add one student at a time."}</p>${additional?`<div class="notice">Adding a second student will require a Family plan when billing is activated.</div>`:""}<div id="childSaveMessage"></div><form id="childSetupForm"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><div class="field"><label for="childFirst">First name</label><input id="childFirst" autocomplete="given-name" required></div><div class="field"><label for="childLast">Last name</label><input id="childLast" autocomplete="family-name" required></div></div><div class="field"><label for="childDob">Date of birth</label><input id="childDob" type="date" required></div><div class="field"><label for="childGrade">Current grade</label><select id="childGrade">${Array.from({length:9},(_,i)=>`<option value="${i+4}">${i+4}</option>`).join("")}</select></div><div class="field"><label for="childExam">Primary test goal</label><select id="childExam"><option value="PSAT">PSAT</option><option value="PSAT/NMSQT">PSAT/NMSQT</option><option value="SAT">SAT</option></select></div><div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn" id="saveAndBilling" type="button">Save & continue to billing</button><button class="btn secondary" id="saveAndAdd" type="button">Save & add another student</button></div></form>`;
}
async function saveChild(profile,container,nextAction){
  ...
  const first_name=container.querySelector("#childFirst")?.value.trim(),last_name=container.querySelector("#childLast")?.value.trim(),date_of_birth=container.querySelector("#childDob")?.value||'',grade_level=Number(container.querySelector("#childGrade")?.value||0)||null,target_exam=container.querySelector("#childExam")?.value||"PSAT";
  if(!first_name||!last_name||!date_of_birth){busy=false;buttons.forEach(b=>{if(b)b.disabled=false;});const m=container.querySelector('#childSaveMessage');if(m)m.innerHTML='<div class="error">Enter the student name and date of birth.</div>';return;}
  const display_name=`${first_name} ${last_name}`;
  try{
    await authedPost('/api/parent-student',{first_name,last_name,date_of_birth,grade_level,target_exam});
  }catch(e){busy=false;buttons.forEach(b=>{if(b)b.disabled=false;});const m=container.querySelector("#childSaveMessage");if(m)m.innerHTML=`<div class="error">${esc(e.message)}</div>`;return;}
  busy=false;
...
async function injectStudentParentInvite(){
  ...
  const {profile}=await getProfile();if(!profile||profile.role!=="student"||profile.household_id)return;const hero=document.querySelector(".hero");if(!hero)return;const card=document.createElement("div");card.id="parentInviteCard";card.className="card";card.style.marginTop="16px";card.innerHTML=`<h2>Invite a parent or guardian</h2><p class="small">Linking a parent lets them activate your trial, manage billing, see your progress and help keep your plan on track.</p><div class="field"><label for="inviteParentEmail">Parent or guardian email</label><input id="inviteParentEmail" type="email" placeholder="parent@example.com" autocomplete="email"></div><div id="parentInviteMessage"></div><button class="btn" id="inviteParentBtn">Invite my parent</button>`;hero.insertAdjacentElement("afterend",card);card.querySelector("#inviteParentBtn").onclick=async()=>{const btn=card.querySelector('#inviteParentBtn'),parent_email=card.querySelector("#inviteParentEmail").value.trim(),msg=card.querySelector('#parentInviteMessage');if(!parent_email)return;btn.disabled=true;btn.textContent='Creating invitation…';try{const result=await authedPost('/api/student-parent-invitation',{parent_email});card.innerHTML=`<div class="success"><strong>${result.existing?'Invitation already pending.':'Invitation created.'}</strong><br>Your parent or guardian can now be linked to your SATprep.io account.</div>`}catch(e){btn.disabled=false;btn.textContent='Invite my parent';msg.innerHTML=`<div class="error">${esc(e.message)}</div>`}};
}
