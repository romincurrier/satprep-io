...
export async function scoreDiagnosticAnswer(student,attemptId,position,submittedResponse,responseMs){const attempt=await ownedAttempt(student.id,attemptId);if(!attempt)throw Object.assign(new Error('Diagnostic attempt not found.'),{status:404});if(attempt.status==='completed')return{ok:true,completed:true};if(!(await contentSystemReady()))throw Object.assign(new Error('The secure diagnostic content system is not ready.'),{status:503});const{ids,p,itemId}=planPosition(attempt,position);await assertPersistedPlanItem(attemptId,p,itemId);const{item,answer}=await approvedContent(itemId),scored=scoreResponse(item.format,answer,submittedResponse);if(!scored.valid)throw Object.assign(new Error(scored.error||'Assessment response is invalid.'),{status:scored.keyInvalid?503:400});
 let written;try{const result=await service('/rest/v1/rpc/submit_diagnostic_response_secure_v3',{method:'POST',body:JSON.stringify({p_attempt_id:attemptId,p_student_id:student.id,p_position:p,p_question_key:item.id,p_selected_answer:scored.selectedAnswer,p_response_text:scored.responseText,p_is_correct:scored.correct,p_response_ms:Math.max(0,Math.min(3600000,Number(responseMs)||0))})});written=Array.isArray(result)?result[0]:result}catch(e){const message=String(e?.message||'');if(message.includes('already submitted')||message.includes('out of sequence')||message.includes('not active'))e.status=409;else if(message.includes('not production-approved')||message.includes('persisted plan'))e.status=503;throw e}
 if(!written?.accepted)throw Object.assign(new Error('The diagnostic response could not be recorded safely.'),{status:503});const total=Number(written.total)||ids.length,count=Number(written.answered)||0,completed=count>=total;if(completed&&!await finalizeSecureDiagnostic(student,attempt))throw Object.assign(new Error('Your answers are saved, but diagnostic finalization could not be confirmed. Retry the same answer to complete your assessment.'),{status:503});return{ok:true,completed,nextPosition:completed?null:count,answered:count,total,idempotent:written.idempotent===true}}
