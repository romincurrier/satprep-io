...
export default async function handler(req,res){
 ...
 try{
  ...
  const subs=await service(`/rest/v1/subscriptions?household_id=eq.${encodeURIComponent(p.household_id)}&select=status&order=created_at.desc&limit=1`);if(!['trialing','active'].includes(subs?.[0]?.status))return json(res,403,{error:'Activate a household trial or subscription before creating the student login.'});
  let created;try{created=await service('/auth/v1/admin/users',{method:'POST',body:JSON.stringify({email:normalized,password,email_confirm:true,user_metadata:{role:'student',first_name:s.first_name||'',last_name:s.last_name||'',household_id:p.household_id}})})}catch(e){if(e.status===422||/already|registered|exists/i.test(String(e.message||'')))return json(res,409,{error:'That email is already associated with a SATprep.io account. Use a different email or sign in with the existing account.'});throw e}
