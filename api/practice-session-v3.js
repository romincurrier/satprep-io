...
export default async function handler(req,res){
 ...
 try{
  ...
  const state=await ensurePracticeSession(ctx.student,skill);return json(res,200,{session_id:state.session.id,skill_key:state.session.skill_key,answered:state.answered,total:state.total,resumed:state.resumed,status:state.session.status,adaptive_level:state.adaptiveBand});
