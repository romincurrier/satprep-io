...
export default async function handler(req,res){
 ...
 try{
  stripeSecret();const auth=await billingContext(req),p=auth?.profile;if(!p)return json(res,401,{error:'Please sign in again.'});if(p.role!=='parent'||!p.household_id||!p.billing_owner)return json(res,403,{error:'Only the household billing owner can manage the subscription.'});
 ...
 }catch(e){console.error('create-portal-session',e);const status=Number(e.status);return json(res,status===503?503:500,{error:status===503?e.message:'Unable to open billing management right now.'})}
}
