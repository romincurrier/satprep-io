...
import {assertBillingPortalEnabled,assertBillingRequestOrigin,billingContext,safeSiteOrigin,stripeRequest,stripeSecret} from '../server/stripe-server.js';

export default async function handler(req,res){
 if(req.method!=='POST')return json(res,405,{error:'Method not allowed'});
 try{
  assertBillingRequestOrigin(req);assertBillingPortalEnabled(req);stripeSecret();const auth=await billingContext(req),p=auth?.profile;if(!p)return json(res,401,{error:'Please sign in again.'});if(p.role!=='parent'||!p.household_id||!p.billing_owner)return json(res,403,{error:'Only the household billing owner can manage the subscription.'});
