...
import {billingContext,stripeRequest,stripeSecret} from '../server/stripe-server.js';

const PLANS=new Set(['individual_monthly','individual_annual','family_monthly','family_annual']);
export default async function handler(req,res){
 if(req.method!=='POST')return json(res,405,{error:'Method not allowed'});
 try{
  stripeSecret();const auth=await billingContext(req),p=auth?.profile;if(!p)return json(res,401,{error:'Please sign in again.'});if(p.role!=='parent'||!p.household_id||!p.billing_owner)return json(res,403,{error:'Only the household billing owner can confirm checkout.'});
 ...
 }catch(e){console.error('confirm-checkout-session',e);const status=Number(e.status);return json(res,status===503?503:500,{error:status===503?e.message:'Unable to confirm checkout right now.'})}
}
