...
export default async function handler(req,res){
 ...
 try{
  ...
  const studentPromise=enrollment.student_id?service(`/rest/v1/students?id=eq.${encodeURIComponent(enrollment.student_id)}&select=id,profile_id,display_name,first_name,last_name,grade_level,target_exam,target_score,onboarding_complete,diagnostic_completed_at,diagnostic_math_mastery,diagnostic_rw_mastery,recommended_path,is_test_student,test_label,created_at&limit=1`):enrollment.household_id?service(`/rest/v1/students?household_id=eq.${encodeURIComponent(enrollment.household_id)}&is_test_student=eq.true&select=id,profile_id,display_name,first_name,last_name,grade_level,target_exam,target_score,onboarding_complete,diagnostic_completed_at,diagnostic_math_mastery,diagnostic_rw_mastery,recommended_path,is_test_student,test_label,created_at&order=created_at.asc&limit=1`):Promise.resolve([]);
