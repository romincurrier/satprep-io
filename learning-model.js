...
const CANON={
  'Vocabulary':'Words in Context','Precision':'Words in Context','Reading Comprehension':'Reading Comprehension','Evidence':'Evidence','Inference':'Inference','Analysis':'Analysis',
...
async function compose(){const p=new URLSearchParams(location.search);if(p.get('app')!=='1')return;const{data:{session}}=await supabase.auth.getSession();if(!session)return;const{data:profile}=await supabase.from('profiles').select('id,role').eq('id',session.user.id).maybeSingle();if(profile?.role!=='student')return;const{data:student}=await supabase.from('students').select('*').eq('profile_id',profile.id).maybeSingle();if(!student)return;
 ...
 const map={};
 for(const e of evidence||[]){if(e.score_kind==='content_mastery')add(map,e.canonical_skill,Number(e.score_value)/100,Math.max(.5,Number(e.reliability||1)),'prior_assessment');else if(e.score_kind==='percentile')add(map,e.canonical_skill,Number(e.score_value)/100,.2*Math.max(.5,Number(e.reliability||1)),'norm_percentile')}
