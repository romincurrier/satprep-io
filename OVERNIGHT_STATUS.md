...
The latest tested main-branch Vercel commit check is **green** after the trademark launch gates, marketing-claims validation, secure MCQ/SPR response work, and expanded regression/security checks.
...
- The production launch validator fails closed if those controls are changed while the College Board trademark/naming review remains unresolved.
...
2. Intentionally reactivate Supabase when approved; apply/reconcile pending migrations and test RLS, service-role, auth-trigger, rate-limit, privacy, content, MCQ/SPR diagnostic, and practice boundaries end to end.
